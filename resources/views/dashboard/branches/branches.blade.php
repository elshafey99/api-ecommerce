        ...
        <table class="table table-hover table-bordered">
            <thead class="table-light">
                <tr>
                    ...
                    <th class="align-middle" style="width: 15%;">{{ __('dashboard.name_ar') }}</th>
                    <th class="align-middle" style="width: 15%;">{{ __('dashboard.name_en') }}</th>
                    <th class="align-middle" style="width: 12%;">{{ __('dashboard.phone') }}</th>
                    <th class="align-middle" style="width: 15%;">{{ __('dashboard.email') }}</th>
                    <th class="text-center align-middle" style="width: 8%;">{{ __('dashboard.orders_count') }}</th>
                    <th class="text-center align-middle" style="width: 8%;">{{ __('dashboard.main') }}</th>
                    <th class="text-center align-middle" style="width: 8%;">{{ __('dashboard.status') }}</th>
                    <th class="text-center align-middle" style="width: 14%;">{{ __('dashboard.actions') }}</th>
                </tr>
            </thead>
            <tbody>
                @if ($data->count() > 0)
                    @foreach ($data as $item)
                        <tr wire:key="branch-{{ $item->id }}" style="height: 80px;">
                            ...
                            <td class="text-center align-middle">
                                <span class="badge rounded-pill bg-info">{{ $item->orders_count }}</span>
                            </td>
                            <td class="text-center align-middle">
                                <div class="form-check form-switch d-flex justify-content-center">
                                    <input class="form-check-input" type="checkbox"
                                        wire:click="toggleMain({{ $item->id }})"
