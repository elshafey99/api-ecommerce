        ...
        <table class="table table-hover table-bordered">
            <thead class="table-light">
                <tr>
                    ...
                    <th class="align-middle" style="width: 8%;">{{ __('dashboard.image') }}</th>
                    <th class="align-middle" style="width: 18%;">{{ __('dashboard.name_ar') }}</th>
                    <th class="align-middle" style="width: 18%;">{{ __('dashboard.name_en') }}</th>
                    <th class="align-middle" style="width: 15%;">{{ __('dashboard.parent') }}</th>
                    <th class="text-center align-middle" style="width: 8%;">{{ __('dashboard.products_count') }}</th>
                    <th class="text-center align-middle" style="width: 8%;">{{ __('dashboard.children_count') }}</th>
            ...
            <tbody>
                @if ($data->count() > 0)
                    @foreach ($data as $item)
                        <tr wire:key="category-{{ $item->id }}" style="height: 80px;">
                            ...
                            <td class="align-middle">
                                @if ($item->image)
                                    <img src="{{ asset($item->image) }}"
                                        alt="{{ $item->getTranslation('name', 'en') }}" class="rounded"
                                        style="width: 50px; height: 50px; object-fit: cover;">
                                @else
                                    <div class="avatar bg-light-success rounded" style="width: 50px; height: 50px;">
                            ...
                            <td class="align-middle">
                                @if ($item->parent)
                                    <span
                                        class="badge rounded-pill bg-light-primary">{{ $item->parent->getTranslation('name', app()->getLocale()) }}</span>
                                @else
                                    <span class="text-muted">{{ __('dashboard.no-parent') }}</span>
                                @endif
                            </td>
                            <td class="text-center align-middle">
                                <span class="badge rounded-pill bg-info">{{ $item->products_count }}</span>
                            </td>
                            <td class="text-center align-middle">
                                <span class="badge rounded-pill bg-secondary">{{ $item->children_count }}</span>
                            </td>
                            <td class="text-center align-middle">
