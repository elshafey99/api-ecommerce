@extends('dashboard.master', ['title' => __('dashboard.brands')])
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h4 class="card-title mb-0">{{ __('dashboard.brands') }}</h4>
                    <button type="button" class="btn btn-primary waves-effect" data-bs-toggle="modal"
                        data-bs-target="#createModal">
                        <i data-feather='plus'></i> {{ __('dashboard.create-brand') }}
                    </button>
                </div>
                @livewire('dashboard.brands.brands')
            </div>
        </div>
    </div>

    @livewire('dashboard.brands.add-brand')
    @livewire('dashboard.brands.update-brand')
@endsection
...
@push('js')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            Livewire.on('cannotDeleteBrand', function() {
                Swal.fire({
                    position: 'top-start',
                    icon: 'error',
                    title: '{{ __('dashboard.cannot_delete_brand') }}',
                    text: '{{ __('dashboard.brand_has_products') }}',
